    ...
    for attempt in range(2):
        try:
            ...
            if h is None or len(h)<60:
                ...
                return None
            c=h["Close"].dropna(); hh=h["High"].dropna()
            l=h["Low"].dropna(); v=h["Volume"].dropna()
            ...
            lod_dist=round((p-float(l.iloc[-1]))/atr_v*100,1)
            ...
            try:
                ...
                float_pct=round(float(fl)/float(so)*100,1) if fl and so else None
                short_float=round(float(sp)*100,1) if sp else None
            except:
